...
- Trainer battle guide: required; workbook location trainers, VS Seeker rematches and intentionally hidden Gym Leader records plus a conservative version-matched Sevii ROM tranche
...
- The workbook contains standard Johto/Kanto acquisition data through 2026-07-01. Post-workbook Sevii Island content requires structured encounter, item and trainer evidence; reviewed ROM-derived tranches are retained with their map, script and trainer-record provenance.
...
- Trainer Battle cards show each Pokémon's six species base stats and BST alongside its documented level. Workbook battles hide empty moves, abilities, held items and natures. Reviewed ROM-derived Sevii battles additionally show exact moves and any held item stored in the trainer record, while leaving abilities and natures unknown. Memorial Pillar presents Will, Koga, Bruno and Karen as separate initial and post-Kanto stages; the stronger stage is unlocked after all eight Kanto Gym Leaders are defeated, as proven by the ROM's stage selector and Gym-victory flag chain.
